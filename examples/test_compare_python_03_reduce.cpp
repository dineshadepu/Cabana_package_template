...
double CreateParticles()
{
  ...
  double reduce_ = 0.;
  CabanaNewPkg::testReduceEquation(*particles, *verlet_list, neighborhood_radius, reduce_);
  // Create and open a CSV file
  std::ofstream file("./reduce_cpp.csv");
  ...
  if (file.is_open()) {
    ...
    // std::cout << "Value saved to output.csv successfully." << std::endl;
  } else {
    std::cerr << "Error: Unable to open file." << std::endl;
  }

  // std::cout << "Total neighbour sum value is " << reduce_ << std::endl;
